...
// Zdjęcie na karcie mDowodu (.user_photo w card.html)
async function applyUserPhoto(image) {
    const photoImg = document.querySelector('.id_own_image .user_photo, .user_photo');
    const holder = document.querySelector('.id_own_image');
    if (!photoImg || !image) {
        return;
    }
    try {
        ...
        console.error('Błąd konwersji zdjęcia:', error);
        photoImg.src = image;
    }
...
function loadAllData() {
    try {
        ...
        // Wczytaj zdjęcie
        if (formData.image) {
            void applyUserPhoto(formData.image);
        }
...
window.applyUserPhoto = applyUserPhoto;

document.addEventListener('DOMContentLoaded', loadAllData);
document.addEventListener('mobywatel-profile-synced', () => {
    loadAllData();
});
